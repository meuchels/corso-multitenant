...
# Define file paths
environment_file="./environments/$SHORT_TENANT_NAME-sharepoint"
backup_file="./back-available/$SHORT_TENANT_NAME-sharepoint"
...
#END EDIT
####################################
export configfile=/opt/corso/toml/${tenantshortname}-sharepoint.toml
EOF
)
...
source /opt/corso/scripts/environments/$TENANT_SHORT_NAME-sharepoint

##############End Edit###


# create runtime variables
logfilename="/opt/corso/log/${tenantshortname}-sharepoint/$(date +'%Y-%m-%d-%H%M%S').log"
runcorso="/opt/corso/corso"

# init bucket
$runcorso repo init s3 --bucket $bucket --prefix ${tenantshortname}_sharepoint --endpoint $s3endpoint --log-file $logfilename --config-file $configfile --hide-progress
$runcorso repo connect s3 --bucket $bucket --log-file $logfilename --config-file $configfile --hide-progress

# Run Backup
$runcorso backup create sharepoint --site '*' --log-file $logfilename --config-file $configfile --hide-progress
EOF
)
